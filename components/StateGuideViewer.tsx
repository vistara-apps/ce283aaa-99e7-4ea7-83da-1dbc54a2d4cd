...
export function StateGuideViewer({ guide, language, onLanguageChange }: StateGuideViewerProps) {
  const [expandedSections, setExpandedSections] = useState<string[]>(['rights']);
                ...
                {section.content.map((item, index) => (
                  <li key={index} className="flex items-start space-x-3">
                    <div className={`w-2 h-2 rounded-full mt-2 ${
                      section.id === 'rights' ? 'bg-green-400' :
                      section.id === 'say' ? 'bg-blue-400' : 'bg-red-400'
                    }`} />

...
export function useRecording(options: UseRecordingOptions = {}) {
  ...
  const startRecording = useCallback(async (userId: string) => {
    try {
      ...
      const incident: RecordedIncident = {
        ...
        location: {
          ...
          address: await getAddressFromCoordinates(location.latitude || 0, location.longitude || 0)
        },
        alertSent: false
      };
  ...
  const sendAlert = useCallback(async () => {
    if (currentIncident) {
      try {
        ...
        if (response.ok) {
          setCurrentIncident(prev => prev ? { ...prev, alertSent: true } : null);
        }
      } catch (error) {
  ...
  const uploadRecording = useCallback(async (blob: Blob): Promise<string | null> => {
    try {
      ...
      formData.append('file', blob, `incident-${currentIncident?.incidentId}.webm`);
  ...
  return {
    ...
    sendAlert,
    uploadRecording,
    hasLocationPermission: location.latitude !== null && location.longitude !== null,
    locationError: location.error
  };
}

async function getAddressFromCoordinates(lat: number, lng: number): Promise<string | undefined> {
  try {

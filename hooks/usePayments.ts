...
export function usePayments(options: UsePaymentsOptions = {}) {
  ...
  const confirmPayment = useCallback(async (
    ...
    try {
      ...
      const { error, paymentIntent } = await stripe.confirmPayment({
        ...
        clientSecret,
        confirmParams: {
          return_url: confirmationData?.return_url || `${window.location.origin}/payment-success`,
  ...
  const purchaseStateGuide = useCallback(async (
    elements: StripeElements,
    userId: string,
    guideId: string,
    amount: number,
  ...
  const getPaymentStatus = useCallback(async (paymentIntentId: string) => {
    try {
      const response = await fetch(`/api/payments/create-intent?paymentIntentId=${paymentIntentId}`);
      const result = await response.json();
